...
#            time_second = (datetime.strptime(tokens[1], '%a %b %d %X +0000 %Y') - datetime(2010, 1, 1)).total_seconds()
            ...
            # 20211205-build category-dict matrix
            if self.poicatg_dict.get(location) is None:
                self.poicatg_dict[location] = [category_layer]
            else:
                self.poicatg_dict[location].append(category_layer)
...
#                user_timeCovert = [time_covert]
        ...
        poicatg_temp = self.poicatg_dict
        self.zero_matrix = np.zeros((len(self.poi2id), len(self.catg2id_layer)),dtype=np.float32)
        number = 0
        for i in poicatg_temp.keys():
            for j in range(len(poicatg_temp[i])):
                number = number + 1
                self.zero_matrix[i][poicatg_temp[i][j]] = 1
